...
pub trait ReserveQueueModule {
    ...
    fn pop(
        ...
        amount: &BigUint,
    ) -> SCResult<Vec<NonceAmountPair<Self::Api>>> {
    ...
    fn push(
        &self,
        token_id: &TokenIdentifier,
        pairs: &[NonceAmountPair<Self::Api>],
    ) -> SCResult<()> {
        let queues_orig = self.queues(token_id).get();
        let mut queues = queues_orig.clone();
        let precision = self.get_precision_or_default(token_id);
    ...
    fn pop_from_queue_non_fungible(
        ...
        let big_zero = BigUint::zero();
        let mut amount_left = amount.clone();
        let mut current_elem: ReserveNode<Self::Api>;
    ...
    fn get_compressed_pairs(
        &self,
        pairs: &[NonceAmountPair<Self::Api>],
    ) -> Vec<NonceAmountPair<Self::Api>> {
        let mut pairs_compressed = Vec::<NonceAmountPair<Self::Api>>::new();
    ...
    fn get_queue_id(&self, amount: &BigUint, precision: u64) -> u64 {
        let mut queue_id = 0;

        let precision_biguint = BigUint::from(precision);
    ...
    fn reserve(
        ...
        token_id: &TokenIdentifier,
        nonce: Nonce,
    ) -> SingleValueMapper<ReserveNode<Self::Api>>;

    #[storage_mapper("head")]
    fn head(&self, token_id: &TokenIdentifier, queue_id: u64) -> SingleValueMapper<Nonce>;

    #[storage_mapper("tail")]
    fn tail(&self, token_id: &TokenIdentifier, queue_id: u64) -> SingleValueMapper<Nonce>;

    #[storage_mapper("queues")]
    fn queues(&self, token_id: &TokenIdentifier) -> SingleValueMapper<Vec<u64>>;

    #[storage_mapper("precision")]
    fn precision(&self, token_id: &TokenIdentifier) -> SingleValueMapper<u64>;
}
