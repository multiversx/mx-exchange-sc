...
pub struct EnergyFactory;

pub trait EnergyFactoryTrait {
    // views
    async fn get_energy_entry_for_user(
        dex_interact: &mut DexInteract,
        user: Address,
    ) -> InteractorEnergy;
    async fn get_energy_amount_for_user(
        dex_interact: &mut DexInteract,
        user: Address,
    ) -> RustBigUint;
    ...
    async fn get_token_unstake_address(dex_interact: &mut DexInteract) -> Address;
    async fn is_sc_address_whitelisted(dex_interact: &mut DexInteract, address: Address) -> bool;
}

impl EnergyFactoryTrait for EnergyFactory {
    async fn get_energy_entry_for_user(
        dex_interact: &mut DexInteract,
        user: Address,
    ) -> InteractorEnergy {
