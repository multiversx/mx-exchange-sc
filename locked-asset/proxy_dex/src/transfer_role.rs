    ...
    proxy_common::ProxyCommonModule + token_send::TokenSendModule
{
    ...
    fn unset_transfer_role_locked_lp_token(
        &self,
        #[var_args] opt_address: OptionalValue<ManagedAddress>,
    ) {
        let locked_lp_token_id = self.wrapped_lp_token_id().get();
    ...
    fn unset_transfer_role_locked_farm_token(
        &self,
        #[var_args] opt_address: OptionalValue<ManagedAddress>,
    ) {
        let locked_farm_token_id = self.wrapped_farm_token_id().get();
