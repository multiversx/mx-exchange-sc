...
fn swap_tokens_proxy_test() {
    ...
    let swap_amount_out = 1_000;
    let swap_amount_in = 1_993;

    setup
        .b_mock
        .set_esdt_balance(&first_user, WEGLD_TOKEN_ID, &(&full_balance - &other_token_amount + &rust_biguint!(swap_amount_out)));
    ...
    setup
        ...
        .execute_esdt_transfer(
            ...
            WEGLD_TOKEN_ID,
            0,
            &rust_biguint!(swap_amount_out),
            |sc| {
                let ret = sc.swap_tokens_fixed_input(
                    managed_token_id!(MEX_TOKEN_ID),
                    managed_biguint!(1_900),
                );

                assert_eq!(ret.token_identifier, managed_token_id!(MEX_TOKEN_ID));
                assert_eq!(ret.token_nonce, 0);
                assert_eq!(ret.amount, managed_biguint!(swap_amount_in));
            },
        )
        .assert_ok();
    ...
    setup
        .b_mock
        .check_esdt_balance(&first_user, MEX_TOKEN_ID, &rust_biguint!(swap_amount_in));

    // check pair balance
    setup.b_mock.check_esdt_balance(
        setup.pair_wrapper.address_ref(),
        MEX_TOKEN_ID,
        &(locked_token_amount.clone() - rust_biguint!(swap_amount_in)),
    );
    // pass epochs to process energy update
    block_epoch = 10u64;
    setup.b_mock.set_block_epoch(block_epoch);
    ...
    let half_lp_tokens = &expected_lp_token_amount / 2u32;
    // should be 500_000_000, but ends up so due to approximations
    let removed_locked_token_amount = rust_biguint!(499_998_003);
    // should be 250_000_000, but ends up so due to approximations
    let removed_other_token_amount = rust_biguint!(249_999_999);
    setup
        .b_mock
        .execute_esdt_transfer(
            &first_user,
    ...
    setup
        ...
        .execute_query(&setup.simple_lock_wrapper, |sc| {
            let unlock_epoch = LOCK_OPTIONS[0];
            let lock_epochs = unlock_epoch - block_epoch;
            let new_balance = user_balance - (swap_amount_in / 2) - 1;
            let expected_energy_amount =
                BigInt::from((new_balance) as i64) * BigInt::from(lock_epochs as i64);
            let expected_energy = Energy::new(
                expected_energy_amount,
                block_epoch,
                managed_biguint!(new_balance),
            );
            let actual_energy = sc.user_energy(&managed_address!(&first_user)).get();
            assert_eq!(expected_energy, actual_energy);
        })
